...
\begin{abstract}
This project investigates automated reasoning for first-order logic using a backward sequent calculus proof search method. The assignment goal was to implement the textbook proof search procedure and then improve its performance on benchmark formulae. The system developed in this project reads formulae from text files, parses each formula into an abstract syntax tree, converts the target formula into an initial sequent, and then attempts to prove it using a baseline solver based on Algorithm 2 from the course textbook. The baseline solver applies sequent calculus rules backwards, closing branches using identity and other closing rules, decomposing non branching connectives first, and then exploring branching and quantifier rules. An improved solver was also developed by adding repeated state caching, deterministic rule ordering, and coordinated quantifier instantiation. These changes are specially designed to reduce repeated exploration and choose useful terms earlier when handling quantified formulae. The system was tested on three generated datasets which are easy, medium, and hard. The results show that both solvers handled simple formulae good, while the improved solver was more useful on difficult formulae involving nested quantifiers and implication chains. The project shows that even small proof search optimisations can also improve the  behaviour of a first order reasoning system, although the method is still limited by the deep bounds and the inconsistency of first order validity.
\keywords{Automated reasoning \and First-order logic \and Sequent calculus \and Proof search}
\end{abstract}

\section{Introduction}

Automated reasoning studies shows how computers can check logical arguments and prove formulae automatically. It is crucial in formal verification, artificial intelligence, program analysis, and security because many true problems can be represented as logical proof tasks. First order logic is especially useful because it can describe objects, predicates, and quantified statements such as ``all objects have a property'' or ``there exists an object with a property''.
...
The baseline is kept simple so that it closely follows the textbook algorithm. It follows the rule order from the textbook and uses a fixed search depth and step limit to avoid infinite search.
...
First, repeated state caching records previously explored sequents together with their quantifier \textbf{}instantiation history. If the same proof state is reached again, the solver avoids repeating the same search path. This is expected to reduce unnecessary recomputation.
...
Three datasets were generated for this project: easy, medium, and hard. Each dataset contains 10 formulae, giving 30 formulae in total. The formulae were designed using the first-order logic syntax and sequent calculus proof-search ideas covered in the course textbook, but they were not copied directly from the book or from an online benchmark library.

The easy dataset contains simple propositional formulae and single quantifier formulae. The medium dataset contains formulae where universal or existential formulae interact with implication. The hard dataset contains nested quantifiers, longer implication chains, and branching formulae. These datasets are suitable because they test different proof-search behaviours, including identity closure, propositional branching, quantifier instantiation, and repeated reasoning across implication chains.

The datasets also include formulae with different levels of difficulty. This helps test both normal successful proof search and cases where the search space becomes larger because of branching or quantifier instantiation.
...
The experiment was run on Windows 11 using Python 3.13.2 on a laptop with a 13th Gen Intel(R) Core(TM) i5-13500H processor at 2.60 GHz and 16 GB RAM at 5200 MT/s.
...
\hline
Dataset & Formulae & Base solved & Imp. solved & Base time & Imp. time & Base steps & Imp. steps\\
\hline
Easy & 10 & 8 & 8 & 0.000709 & 0.000862 & 3.8 & 3.8\\
Medium & 10 & 10 & 10 & 0.003533 & 0.008616 & 13.2 & 13.2\\
Hard & 10 & 9 & 10 & 0.096829 & 0.162230 & 56.1 & 93.6\\
\hline
\end{tabular}
\end{table}
The step columns show the average number of proof-search states visited by each solver for formulas in that dataset.
...
The source code and datasets for this project are available in the project GitHub repository:
\href{https://github.com/Maxbatan/3806ICT_Assignment1Vigneshakamax}{GitHub repository for this project}.

\begin{thebibliography}{8}

\bibitem{hou2021}
Hou, Z.:
\textit{Fundamentals of Logic and Computation: With Practical Automated Reasoning and Verification}.
Springer, Cham (2021).
...
\textit{Journal of Automated Reasoning} 59, 483--502 (2017).
